...
import { Listbox, ListboxItem, cn, Button, Link } from "@heroui/react";
      ...
      <ul className="side-navigation">
        {siteConfig(loginUser).navItems.filter(x => x.isShow).map((item, i) => (
          <li
            ...
            className={`${pathname === item.href ? "active" : ""} flex-col text-left justify-start`}
          >
            <div className="flex">
            ...
            <div className="flex flex-col">
              {item?.subMenu?.map((sub, i) => {
                return <Link key={i} onPress={() => router.push(sub.href)} className="p-2 ml-6 text-sm">{sub?.label}</Link>
              })}
            </div>
          </li>
        ))}
